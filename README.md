...
0.0.7 は**起動した時点で保存データを書き換えます**。画面が出るより前に済んでいるので、あとから取り消すことはできません。
...
なお 0.0.6 にはデータを消す機能がありません。0.0.7 の **設定 → すべて消す** は、0.0.6 から持ち越したデータにも使えます。
...
- `%AppData%\rescicle` — 設定、Research Objectと会話を保持するSQLiteデータベース、エージェント作業ディレクトリ。中身だけ空にしたいなら、アンインストール前に **設定 → すべて消す** が使えます。
...
2. 既存の研究フォルダを選びます。rescicleはファイルを読むだけで、書き換え・移動・コピー・削除はしません。
...
8. 手元の既存ファイルをAssetとして登録できます。**データとして登録** を押した時点で登録は終わりです。そのあと、それを生み出した測定を選べます。選ぶと線が引かれ、測定は実施済みになります。選ばなくても登録は済んでいます。
9. **Ctrl+F** で検索、**Ctrl+K** で会話欄へ。Escapeで戻ります。
10. 右上の **設定** に、Claude Codeの接続状態、研究フォルダの変更、データのリセットがあります。フォルダを変えても中身は動きません。リセットはrescicleが記録したものを全部消して最初の画面に戻すもので、研究フォルダのファイルには触れません。
