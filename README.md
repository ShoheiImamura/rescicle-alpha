...
- Claude CodeへのMCP追加（追加した場合）。消さないと、Claude Codeが `rescicle` サーバーの起動失敗を報告し続けます:
...
3. 最初の応答が返るころ、**Claude Code側からもrescicleを読み書きできます** という案内が出ることがあります。使うなら **追加する**、使わないなら **あとで**。この端末で一度追加すれば、もう出ません。
...
追加は **設定 → Claude Codeに追加** で終わります。rescicleが `claude` CLIを自分で実行するので、ターミナルを開く必要はありません。すでに追加されているかどうかも同じ画面が読み戻して表示します。

同じ名前への再追加はCLIに拒否されるため、追加は毎回いったん外してから入れ直します。古いビルドのパスが残っている場合も、これで現在のrescicleに置き換わります。実行されるのは次と同等です:
...
自分でターミナルから実行したい場合は、同じ画面の **追加コマンドをコピー** が使えます。
...
各Research Object / Relationは `origin` と `status` を持ちます。Objectは `proposed` / `confirmed` / `rejected` / `archived` の4つ、Relationは `archived` を除く3つです（線は構造なので、済んだものは片付けるのではなく外します）。
